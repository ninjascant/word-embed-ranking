...
class TextCleanerTest(unittest.TestCase):
    ...
    def test_remove_pronouns():
        ...
        cleaned_preprocessed_texts = lemmatizer_with_clean.transform(cleaned)

        lemmatizer_without_clean = Lemmatizer(remove_pronouns=False)
        preprocessed_texts = lemmatizer_without_clean.transform(cleaned)

        cleaned_has_not_pron = ['-PRON-' not in text for text in cleaned_preprocessed_texts]
        preprocessed_has_pron = ['-PRON-' in text for text in preprocessed_texts]
